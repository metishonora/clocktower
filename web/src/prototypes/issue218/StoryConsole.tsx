...
function Board({ full = false, players, step, choosing, targets, inspected, onTarget, onInspect, onExpand }: BoardProps) {
    ...
    return <section className={`seatingLeaf paper ${full ? 'fullBoard' : ''}`} aria-label="마도서 배치">
      <header className="leafMasthead"><span>이야기꾼의 마도서</span><span>배치</span></header>
      <div className="boardTitle"><h2>이 밤의 사람들</h2><span><b>{players.filter(p => p.alive).length}</b> 생존 <i>/</i> {players.length}명</span></div>
      <div className="seatingBoard" style={{ '--board-height': players.length > 12 ? '1040px' : '590px' } as CSSProperties}>
        <div className="boardCenter"><Sigil/><div><span>등불 아래의 속삭임</span><strong>첫날 밤</strong><small>{choosing ? '대상 두 명 선택' : step < 4 ? `${stepLabels[step]}의 차례` : '전달 완료'}</small>{choosing && <b className="targetCount">{targets.length} / 2</b>}</div></div>
...
function StoryConsole() {
  ...
  if (disclosure) return <div className="disclosureRoot"><main className={`disclosurePaper paper ${covered ? 'isCovered' : ''}`} aria-label="플레이어 공개 화면">
    <header className="leafMasthead"><span>밤의 속삭임</span><span>Ⅰ</span></header>
    ...
    <main>
      <div hidden={view !== 'story'} className="spread">
        ...
        <section className="chronicleLeaf paper" aria-label="진행과 기록">
          <header className="leafMasthead"><span>등불 아래의 속삭임</span><button className="textButton" aria-expanded={history} onClick={() => history ? returnCurrent() : showHistory()}>{history ? '현재 진행으로 ↵' : '기록 목차 ☷'}</button></header>
          {history ? <div className="archive"><div className="chapterHeading"><span className="eyebrow">지난 장을 펼치다</span><h1>밤과 낮의 기록</h1></div><button className="currentBookmark" onClick={returnCurrent}>Ⅰ 첫날 밤 <span>현재 진행으로 ↵</span></button>{archive.map(chapter => <details key={chapter.number} open><summary><span>{chapter.number}</span>{chapter.title}<small>{chapter.rows.length}개의 기록</small></summary>{chapter.rows.map(([who, text, outcome], i) => <article key={i} className="archiveEntry"><strong>{who}</strong><p>{text}</p><span>{outcome}</span></article>)}</details>)}<button className="inkButton" onClick={returnCurrent}>현재 진행으로 ↵</button></div> : <>
            <div className="chapterScene"><img className="nightWoodcut" src={nightWoodcut} alt=""/><div className="chapterHeading"><span className="eyebrow">제 일 장</span><h1>첫날 밤</h1><svg className="titleStroke" viewBox="0 0 260 16" aria-hidden="true"><path d="M3 8Q77 3 139 8T255 6M15 11Q139 5 248 10"/></svg><p>밤이 저물고,<br/>마을은 깊은<br className="mobileIntroBreak"/> 잠에 들었다.</p></div></div>
            ...
            <div className="storyBody">
              {entries.map(entry => <article key={entry.step} className="writtenEntry"><div className="entryOrdinal">0{entry.step + 1}<span>✓</span></div><div><p><em className="recordRole">{entry.role}</em> <strong>{entry.actor}</strong>{entry.step === 0 ? particle(entry.actor, '이 ', '가 ') : particle(entry.actor, '은 ', '는 ')}{entry.text}</p><p className="entryResult">{entry.result}</p><button className="textButton" onClick={() => openDisclosure({ title: entry.revealTitle, text: entry.revealText, bluffs: entry.bluffs })}>전달한 정보 보기 ↗</button></div></article>)}
              <section ref={current} tabIndex={-1} className="currentPassage" aria-label="현재 진행"><div className="passageHeading" aria-label={step < 4 ? '현재 작성 중' : '완료'}><span className="inkDiamond">✧</span></div>
                {step === 0 && <><p className="narrative"><em className="roleInscription">하수인</em> {person(2)}{particle(players[1].name, '과', '와')} {person(3)}{particle(players[2].name, '이', '가')} 눈을 뜨고,<br className="desktopBreak"/> 어둠 속에서 악마를 찾는다.</p><p className="whisperPreview">그들의 악마는 {person(1)}.<span className="whisperAnnotation">{character(players[0].role).label}</span></p></>}
                {step === 1 && <><p className="narrative"><em className="roleInscription">악마</em> {person(1)}에게 하수인과<br/>세 가지 속임수를 알려준다.</p><div className="selectionHeader"><span>속임수 세 가지를 적는다.</span><b>{bluffs.length} / 3</b></div><div className="bluffGrid" role="group" aria-label="속임수 배역 선택">{bluffOptions.map(c => <button key={c.id} aria-pressed={bluffs.includes(c.id)} disabled={bluffs.length === 3 && !bluffs.includes(c.id)} onClick={() => { setBluffs(list => list.includes(c.id) ? list.filter(id => id !== c.id) : [...list, c.id]); markChanged(); }}><span><img src={c.image} alt=""/>{bluffs.includes(c.id) && <b>✓</b>}</span>{c.label}</button>)}</div></>}
                {step === 2 && <><p className="narrative"><em className="roleInscription">{character(actor!.role).label}</em> {person(4)}{particle(players[3].name, '이', '가')} 눈을 뜬다.<br/>이웃한 악한 이들은 몇 쌍인가.</p><div className="numberInput"><button aria-label="쌍 수 줄이기" disabled={chefNumber === 0} onClick={() => { setChefNumber(v => v - 1); markChanged(); }}>−</button><output aria-label="전달할 쌍 수">{chefNumber}<small>쌍</small></output><button aria-label="쌍 수 늘리기" disabled={chefNumber === players.length} onClick={() => { setChefNumber(v => v + 1); markChanged(); }}>+</button></div><p className="inputHint">이야기꾼이 전달할 수를 정합니다.</p></>}
                {step === 3 && <><p className="narrative"><em className="roleInscription">{character(actor!.role).label}</em> {person(5)}{particle(players[4].name, '이', '가')} 두 사람의<br/>운명을 들여다본다.</p><div className="selectionHeader"><span>들여다볼 두 사람은,</span><b>{targets.length} / 2</b></div><div className="chosenTargets">{[0, 1].map(i => <button key={i} className={targets[i] ? 'hasTarget' : ''} aria-label={targets[i] ? `${players.find(p => p.id === targets[i])!.name} 선택 해제` : `대상 ${i + 1} 선택`} onClick={() => targets[i] ? selectTarget(targets[i]) : setSheet(true)}>{targets[i] ? <><img src={character(players.find(p => p.id === targets[i])!.role).image} alt=""/><strong>{players.find(p => p.id === targets[i])!.name}</strong><span>×</span></> : <><span>＋</span>배치에서 선택</>}</button>)}</div><fieldset className="resultChoices"><legend>그중에 악마가 있는가?</legend>{['예', '아니오'].map(r => <button type="button" key={r} aria-pressed={result === r} onClick={() => { setResult(r); markChanged(); }}>{r}</button>)}</fieldset></>}
      ...
      <section hidden={view !== 'seating'} className="standaloneBoard"><div className="standaloneHeading"><div><span className="eyebrow">이야기꾼의 마도서</span><h1>배치를 펼치다</h1></div><button className="darkTextButton" onClick={() => navigate('story')}>진행으로 돌아가기 ↵</button></div><Board {...boardProps} full/>{choosing && <div className="standaloneSelection"><span>{actor?.name} · {character(actor?.role ?? 'fortuneTeller').label} · 대상 {targets.length} / 2</span><button className="inkButton" onClick={() => navigate('story')}>진행으로 ↵</button></div>}</section>
      <section hidden={view !== 'storage'} className="storageLeaf paper" aria-label="저장 화면"><header className="leafMasthead"><span>이야기꾼의 마도서</span><span>보관</span></header><div className="chapterHeading"><span className="eyebrow">책을 덮기 전에</span><h1>기록을 보관하다</h1><p>등불 아래의 속삭임 · 첫날 밤</p></div><div className="storageSummary"><Sigil/><div><strong>{players.length}명의 이름, {entries.length}개의 기록</strong><p>{savedAt ? `${savedAt} 저장${changed ? ' · 이후 변경 있음' : ''}` : '아직 저장하지 않은 기록'}</p></div></div><div className="storageColumns"><section><span className="eyebrow">한 장을 남기다</span><h2>기기에 저장</h2><p>현재 배치와 진행을 보관합니다.</p><button className="inkButton" onClick={saveDevice}>기기에 저장 ↓</button><div className="savedSlot"><span>{slot ? '등불 아래의 속삭임' : '보관된 기록 없음'}</span>{slot && <><small>첫날 밤 · {slot.snapshot.entries.length}개의 기록 · {slot.time}</small><button className="textButton" onClick={() => setPendingLoad(slot.snapshot)}>이 기록 불러오기 ↗</button></>}</div></section><section><span className="eyebrow">기록을 옮기다</span><h2>파일로 보관</h2><p>게임 기록을 파일로 옮깁니다.</p><button className="outlineButton" onClick={() => { setFile(snapshot()); setNotice('파일 저장 화면을 확인했습니다. 실제 파일은 생성하지 않습니다.'); }}>파일로 저장 ↓</button><button className="textButton fileLoad" disabled={!file} onClick={() => file && setPendingLoad(file)}>파일 불러오기 ↗</button></section></div><footer className="leafFolio"><button className="textButton" onClick={() => navigate('story')}>진행으로 돌아가기 ↵</button><span>— 보관 —</span></footer></section>
    </main>
