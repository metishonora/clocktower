...
import '../features/madness/madnessActionDock.css';
...
export function MadnessActionView({
  ...
  renderIdentity: IdentityRenderer;
    ...
    <section className={`snvMadnessPanel ${theme}`} aria-label={`${targetLabel} 집착 확인`}>
      <header>
        <div className="snvMadnessHeaderContent">
          {renderIdentity(assignment.sourceCharacterId,theme,<>            {sourceAsset ? <img src={sourceAsset.src} alt={`${sourceLabel} 공식 캐릭터 아이콘`} /> : null}
            <div>
...
function MadnessExecutionDialog({ assignment, target, theme, busy, onCancel, onConfirm, description }: {
  description:string;
  assignment: MadnessAssignmentState;
  target?: Player;
  theme: "day" | "night";
