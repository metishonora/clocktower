...
// Presentation-only data. No engine/session/catalog dependency is allowed in this view.
type RevealPlayer = { playerId: string; seat: number; name: string };

type RevealIdentity = { seat: number; name: string };


type EvilInformationRevealPayload =
  | {
...
type SetupInformationRevealPayload =
  | {
...
type NumericInformationRevealPayload = {
...
type BooleanInformationRevealPayload = {
...
type FortuneTellerInformationRevealPayload = {
...
type CharacterInformationRevealPayload = {
...
type DreamerInformationRevealPayload = {
  kind: "dreamerInformation";
  characterIds: [string, string];
};


type SeamstressInformationRevealPayload = {
...
type SageInformationRevealPayload = {
  kind: "sageInformation";
  candidatePlayers: [RevealPlayer, RevealPlayer];
};


type CharacterChangeRevealPayload = {
...
type MadnessAssignmentRevealPayload = {
...
type EvilTwinPairRevealPayload = {
  kind: "evilTwinPair";
  players: Array<{
    playerId: string;
    seat: number;
...
type RoleInformationRevealPayload =
...
type TextRevealPayload = { messageKo: string; labelKo?: string; valueKo?: string };
type RevealPayload = TextRevealPayload | RoleInformationRevealPayload | EvilTwinPairRevealPayload | {kind:"spyGrimoire"};
type ScalarInformationCharacterId = NumericInformationRevealPayload["characterId"] | BooleanInformationRevealPayload["characterId"];

export type RoleRevealPresentation = RevealPayload;
type SurfaceProps = {dialogLabel:string;backdropAriaLabel?:string;className?:string;closeLabel:string;onClose:()=>void;children:ReactNode};
export type RoleRevealAssets = {
 label:(id:string)=>string;
 icon:(id:string, className?:string, decorative?:boolean)=>ReactNode;
 scalarLabel:(id:ScalarInformationCharacterId)=>string;
 scalarValue:(id:ScalarInformationCharacterId,value:number|boolean)=>string;
 Surface:ComponentType<SurfaceProps>;
};
const CLOSE_LABEL = "확인했으면 눈을 감으세요";

export function RoleRevealContent({payload,assets:values,onClose}:{payload:RoleRevealPresentation;assets:RoleRevealAssets;onClose:()=>void}){
 const characterLabel=values.label;
 const scalarInformationLabel=values.scalarLabel;
 const scalarInformationValueLabel=values.scalarValue;
 const RevealSurface=values.Surface;
 const CharacterIcon=({characterId,className,decorative}:{characterId:string;className?:string;decorative?:boolean})=>values.icon(characterId,className,decorative);
 return <RevealScreen payload={payload} onClose={onClose}/>;
