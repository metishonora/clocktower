...
export function CustomDayTask({controller,onChooseConsequence}:{controller:FirstNightController;onChooseConsequence:()=>void}) {
  ...
  if(day.pendingGameEnd)return <article className="snvCurrentStep customDayTask"><h3>{day.pendingGameEnd.winningAlignment==='good'?'선팀 승리':'악팀 승리'}</h3><p>{endReason(day.pendingGameEnd.reason)}</p></article>;
  const consequence=day.consequences.find(c=>!c.resolved&&c.source.characterId!=='barber');
  if(consequence&&['sweetheart','klutz'].includes(consequence.source.characterId)){
    const role=characterPresentation(consequence.source.characterId)!;
    return <RoleInformationTaskView className="bmrCurrentStep" ariaLabel={`${role.label} 능력`} busy={busy} needsTargets={!consequence.impairedAtDeath} onChooseTargets={onChooseConsequence} revealed={false}
      identity={<><div className="snvCurrentStepIdentity snvInformationIdentity"><img src={role.image} alt=""/><div><span className="snvCurrentStepRoleName" role="heading" aria-level={3}>{role.label}</span><strong>{person(consequence.source.ownerPlayerId)}</strong></div></div><p className="snvInformationAbility">{role.ability}</p></>}
      revealLabel="효과 없음 확인" onReveal={()=>submit({kind:'resolveConsequence',consequenceId:consequence.id,playerId:null})}>
      {consequence.impairedAtDeath&&<p>사망 당시 취함·중독 · 효과 없음</p>}
