...
export function CustomGrimoirePlay({ controller, onNewGame, onNewScenario, onImport, onRestart }: { controller: FirstNightController; onRestart?: () => void } & CustomUtilityActions) {
  ...
  const runtime=usePhaseRuntime({activePhase:replay.gameEnd?undefined:{key:replay.phase,label:replay.day?`${replay.day.day+(replay.phase==='night'?1:0)}일차 ${replay.phase==='day'?'낮':'밤'}`:'첫날 밤'},gameSessionRevision:0,clock:browserRuntimeClock});
  ...
  useEffect(()=>{
    ...
    else if(state.handoff?.stage==='notification')handoffDestination.current=actionAdapter(state.handoff.step)?.closeDestination??'progress';
