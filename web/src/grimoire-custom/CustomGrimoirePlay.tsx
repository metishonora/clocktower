...
export function CustomGrimoirePlay({ controller, onNewGame, onNewScenario, onSavedGames, onImport, onRestart }: { controller: FirstNightController; onRestart?: () => void } & CustomUtilityActions) {
  ...
  const progressNotification=!!marionetteProgressNotification(state);
  useEffect(()=>{
    ...
    if(active){utilities.close();setTab(progressNotification?'play':'seating');}
  ...
  const originalDraft={playerCount:players.length,selectedIds:players.map(p=>p.actualCharacter),players,setupChoiceId:original?.type==='setupConfirmed'?original.payload.setupChoiceId:undefined,boffinAbility:original?.type==='setupConfirmed'?original.payload.boffinAbility:undefined,marionetteCharacter:players.find(p=>p.actualCharacter==='marionette')?.shownCharacter};
