...
export type ScriptSource = 'troubleBrewing' | 'sectsAndViolets' | 'carousel';
...
export const sourceLabels: Record<ScriptSource, string> = { troubleBrewing: 'TB', sectsAndViolets: 'S&V', carousel: '캐러셀' };
...
export const catalog: CatalogCharacter[] = customScriptCharacters.map(({ id, kind }) => {
  ...
  if (!entry || (entry.source !== 'troubleBrewing' && entry.source !== 'sectsAndViolets' && entry.source !== 'carousel')) throw new Error(`Missing custom presentation: ${id}`);
