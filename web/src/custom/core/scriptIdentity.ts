...
export function replayStateScriptReference(state: ReplayState): ScriptReference { return structuredClone(state.script); }
export function sameScriptReference(left: ScriptReference, right: ScriptReference): boolean { return left.type === "custom" && right.type === "custom" && sameCustomScriptDefinition(left.definition, right.definition); }
