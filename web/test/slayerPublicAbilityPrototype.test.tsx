...
test("opens the public ability only from the eligible Slayer icon", async () => {
  ...
  await user.click(screen.getByRole("button", { name: "3번 서연 학살자 능력 사용" }));
  const dialog = screen.getByRole("dialog", { name: "학살자 능력 사용" });
  expect(within(dialog).getByText("3번 서연 · 학살자")).toBeTruthy();
  expect(within(dialog).getByText("확정하면 결과와 관계없이 이 플레이어의 능력이 소모됩니다.")).toBeTruthy();
  expect((within(dialog).getByRole("button", { name: "학살자 사용 확정" }) as HTMLButtonElement).disabled).toBe(true);
});
...
test("records a miss as spent while keeping Discussion active", async () => {
  ...
  await user.click(within(dialog).getByRole("button", { name: "학살자 사용 확정" }));
  ...
  expect((screen.getByRole("button", { name: "3번 서연 학살자 능력 사용 불가" }) as HTMLButtonElement).disabled).toBe(true);
});
...
test("routes a successful shot to a distinct Death follow-up", async () => {
  ...
  await user.click(within(dialog).getByRole("button", { name: "학살자 사용 확정" }));
  const followUp = screen.getByLabelText("학살자 사망 후속");
...
test("disables the Slayer icon outside Discussion and after use", async () => {
  ...
  await user.click(screen.getByRole("button", { name: "지명 단계" }));
  expect((screen.getByRole("button", { name: "3번 서연 학살자 능력 사용 불가" }) as HTMLButtonElement).disabled).toBe(true);
