...
test("selects a complete roster, shows the active character detail, and advances to seating", async () => {
  ...
  const stageNavigation = within(prototype).getByRole("navigation", { name: "작업 단계" });
  expect(within(utilityNavigation).getByRole("button", { name: "저장 / 불러오기" })).toBeTruthy();
  expect(within(stageNavigation).queryByRole("button", { name: "저장 / 불러오기" })).toBeNull();
...
test("assigns roles to Grimoire seats in role-first and seat-first order", async () => {
  ...
  const editingToolbar = within(seating).getByLabelText("마도서 배치 도구");
  const editBack = within(editingToolbar).getByRole("button", { name: "직업 선택으로 돌아가기" });
  ...
  expect(within(seating).queryByLabelText("좌석 편집기")).toBeNull();
  ...
  expect(seatingTray.classList.contains("contentHeight")).toBe(true);
  expect(seatingTray.querySelector(":scope > header")).toBeNull();
  ...
  expect(within(seatingTray).getByLabelText("좌석 편집기").classList.contains("compactTwoRow")).toBe(true);
  ...
  const editingAlignment = within(inspectorHeader).getByLabelText("선한 진영");
  expect(editingAlignment.textContent).toBe("선");
...
test("turns a confirmed Grimoire into a live reference surface with seat details", async () => {
  ...
  const confirmedBack = within(seating).getByRole("button", { name: "배치로 돌아가기" });
  expect(confirmedBack.textContent).toContain("←");
  expect(confirmedBack.classList.contains("destructive")).toBe(true);
  ...
  const liveAlignment = within(details).getByLabelText("선한 진영");
  expect(liveAlignment.classList.contains("snvAlignmentIcon")).toBe(true);
...
test("distributes fifteen seats around non-overlapping desktop and mobile rectangular perimeters", async () => {
  ...
  expect(seatPanel.classList.contains("contentHeight")).toBe(true);
  expect(seatPanel.querySelector(":scope > header")).toBeNull();
...
test("starts the first night with the always-present evil information steps when no acting role is selected", async () => {
  ...
  const day = within(prototype).getByRole("region", { name: "공개 토론" });
  ...
  expect(within(day).getByRole("heading", { name: "2일차 낮" })).toBeTruthy();
  expect(within(day).getByRole("heading", { name: "공개 토론" })).toBeTruthy();
  expect(within(day).getByRole("button", { name: "낮 종료" })).toBeTruthy();

  await user.click(within(day).getByRole("button", { name: "낮 종료" }));
  expect(within(day).getByRole("heading", { name: "2일차 낮 종료" })).toBeTruthy();
