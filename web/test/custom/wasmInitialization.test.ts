...
beforeEach(() => { vi.resetModules(); state.fail = true; state.calls = 0; state.replay = JSON.stringify({ ok: true, value: {...trace.trace[0]!.replay,nightNumber:0,actionExecutions:[],latestUndoUnit:null} }); });
...
test("failed custom initialization clears its own pending request and permits a later retry", async () => {
  ...
  state.fail = false;
  expect(await client.replay(game)).toEqual({ ok: true, value: {...trace.trace[0]!.replay,nightNumber:0,actionExecutions:[],latestUndoUnit:null} });
