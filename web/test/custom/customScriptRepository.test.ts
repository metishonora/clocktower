...
import type { StoredCustomScriptDefinition } from "../../src/custom/storage/definitionRepository.js";
...
test("definition writes and recovery preserve official game and web-session records in the shared database", async () => {
  ...
  const repository = createTestCustomScriptRepository(idb);
  await putRaw(idb, "latest:troubleBrewing", officialGame);
  await putRaw(idb, "session:troubleBrewing", officialSession);
  ...
  assert.deepEqual(await readRaw(idb, "latest:troubleBrewing"), officialGame);
  assert.deepEqual(await readRaw(idb, "session:troubleBrewing"), officialSession);
});
