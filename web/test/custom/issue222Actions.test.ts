...
it('a late Barber offers only legal counts, a Core-projected chooser and sequential identity notifications',async()=>{
 ...
 c.finishHandoff();c.beginSelection();c.togglePlayer('p1');expect(c.selectionReady).toBe(false);c.togglePlayer('p3');expect(c.selectionReady).toBe(false);
 c.updateInput({chooserPlayerId:'p8'});expect(c.selectionReady).toBe(true);await c.acceptSelection();expect(c.getSnapshot().error).toBeUndefined();
...
it('Vigormortis death consequence uses poison candidates from the Core',async()=>{
 ...
 const allowed=c.step!.requiredInput.allowedPlayerIds!;expect(allowed.length).toBeGreaterThan(0);await choose(c,[allowed[0]]);expect(c.getSnapshot().error).toBeUndefined();c.dispose();
});
