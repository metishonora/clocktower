...
export function otherOrderFor(ids: readonly string[]): FirstNightOrderPlan {
    ...
    catalog.splice(1,0,['preacher','choosePlayer']);
    catalog.splice(catalog.findIndex(([id])=>id==='mathematician'),0,['chambermaid','learnCount']);
    catalog.splice(catalog.findIndex(([id])=>id==='butler'),0,['balloonist','learnPlayer'],['nightwatchman','choosePlayer']);
