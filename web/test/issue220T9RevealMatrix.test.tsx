...
import {isReadableReveal} from '../src/grimoire-custom/CustomReadableReveal';
...
it.each(cases)('T9-6: $name uses a closable payload-only public surface',({name,payload,source})=>{
 ...
 expect(dialog.querySelectorAll('button')).toHaveLength(isReadableReveal(payload)?3:1);
 const firstClose=screen.getByRole('button',{name:source==='twin'?'확인했으면 다음 단계로':'확인했으면 눈을 감으세요'});
 expect(firstClose).toBe(document.activeElement);
 expect(Array.from(dialog.querySelectorAll('img')).every(img=>!!img.getAttribute('src'))).toBe(true);
 const custom=dialog.parentElement!.outerHTML;
 if(source==='twin'){fireEvent.click(firstClose);expect(close).not.toHaveBeenCalled();fireEvent.click(screen.getByRole('button',{name:'두 쌍둥이에게 공개'}));}
 fireEvent.click(screen.getByRole('button',{name:'확인했으면 눈을 감으세요'}));expect(close).toHaveBeenCalledOnce();cleanup();expect(root.style.visibility).toBe('');root.remove();
 ...
 const original=screen.getByRole('dialog');if(!isReadableReveal(payload))expect(dialog.textContent).toBe(original.textContent);
