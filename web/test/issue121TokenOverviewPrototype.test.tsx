...
test("pins complete tokens with their source character icons inside player details", async () => {
  ...
  expect(within(grimoire).getByRole("button", { name: /6번 유나 좌석, 꿈꾸는 자, 토큰 4개/ })).toBeTruthy();
  expect(within(grimoire).getAllByText("+1").length).toBeGreaterThan(0);
  expect(within(grimoire).getByText("+4")).toBeTruthy();
  ...
  const tokens = within(detail).getByRole("list", { name: "부착된 토큰 4개" });
...
test("reviews four, six, and nine tokens without truncating their detail lists", async () => {
  const user = userEvent.setup();
  render(<Issue121TokenOverviewPrototype />);
  ...
  for (const [seat, name, count] of [
    [6, "유나", 4],
    [10, "민재", 6],
    [15, "태윤", 9],
  ] as const) {
    const seatButton = within(grimoire).getByRole("button", {
      name: new RegExp(`${seat}번 ${name} 좌석, .*토큰 ${count}개`),
    });
    expect(within(seatButton).getByText(`+${count}`)).toBeTruthy();

    await user.click(seatButton);
    const detail = within(prototype).getByRole("dialog", { name: `${seat}번 ${name} 플레이어 상세` });
    const tokens = within(detail).getByRole("list", { name: `부착된 토큰 ${count}개` });
    expect(within(tokens).getAllByRole("listitem")).toHaveLength(count);
...
test("hides token affordances and player details during nomination, voting, and attack work", async () => {
  ...
  expect(within(grimoire).queryByText("+4")).toBeNull();
