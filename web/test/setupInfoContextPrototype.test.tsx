...
test("compares three Storyteller-only candidate context placements", async () => {
  ...
  expect(within(comparison).getByText("실제: 주정뱅이")).toBeTruthy();
  ...
  expect(within(drunkCandidate).getByText("실제: 주정뱅이")).toBeTruthy();
...
test("constrains options to selected Actual Characters and removes Storyteller context from Reveal", async () => {
  ...
  expect(within(characterSelect).getAllByRole("option").map((option) => option.textContent)).toEqual(["주정뱅이", "성자"]);
  ...
  expect(within(reveal).getByText("사서 정보: 3번 서연 또는 5번 하린 중 한 명은 주정뱅이입니다.")).toBeTruthy();
  expect(screen.queryByText("실제: 주정뱅이")).toBeNull();
  expect(screen.queryByText("본인 인식: 수도사")).toBeNull();
  expect(screen.queryByText("마도서 · 이야기꾼 전용")).toBeNull();
});
