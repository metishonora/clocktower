...
for(const width of [320,390])test(`T10-1: demon reveal keeps every bluff inside its mobile card at ${width}px`,async({page})=>{
 ...
 await page.getByRole('button',{name:'정보 공개',exact:true}).click();await expect(page.getByRole('dialog',{name:'플레이어 정보'})).toBeVisible();
 await page.screenshot({path:test.info().outputPath('demon-reveal.png'),fullPage:true});
 const images=await page.locator('.bmrRevealBluffs img').evaluateAll(nodes=>nodes.map(n=>{
  const image=n.getBoundingClientRect(),card=n.closest('article')!.getBoundingClientRect(),dialog=n.closest('[role=dialog]')!.getBoundingClientRect();
  return {width:image.width,cardWidth:card.width,insideDialog:image.left>=dialog.left-1&&image.right<=dialog.right+1};
 }));
 expect(images).toHaveLength(3);
 // Functional overflow boundary. Exact BMR medallion size remains a direct visual comparison.
 for(const image of images){expect.soft(image.width).toBeGreaterThan(0);expect.soft(image.width).toBeLessThanOrEqual(image.cardWidth+1);expect.soft(image.insideDialog).toBe(true);}
});
