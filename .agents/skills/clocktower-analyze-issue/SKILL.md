...
4. Write the complete analysis report and `analyze / waiting-for-user` checkpoint to the plan. Do not reduce the saved analysis to decisions or checkpoint metadata.
...
Before sending each report, keep the issue plan synchronized with the same substantive content:

- current behavior and problem;
- requested scope and explicit non-goals;
- affected ownership boundaries, dependencies, and risks;
- decisions, recommendations, user answers, and remaining questions.

After clarification, also record the finalized requirements, acceptance criteria, and whether the UI prototype is required or explicitly skipped. Preserve useful analysis history when updating conclusions.

Keep this record in the problem and requirements space. Do not define the solution architecture, file-by-file changes, test design, or implementation sequence here; `$clocktower-issue` writes those details after the prototype is approved or skipped.

## Continue clarification

1. Apply the user's answers and the resulting analysis changes to the plan, then recheck affected facts.
