...
fn twin() -> Value {
    super::issue207_relationships::game("evilTwin")
}
fn execution(state: &Value) -> &Value {
    assert!(
        state.get("actionExecutions").is_some(),
        "T13 P1: missing Core actionExecutions"
    );
    assert!(
        state.get("latestUndoUnit").is_some(),
        "T13 P1: missing Core latestUndoUnit"
    );
...
fn t13_dependency_parent_is_the_confirmed_assignment_and_shared_with_undo() {
    ...
    assert_eq!(
        complete["latestUndoUnit"]["eventIds"],
        json!([assignment["id"], informed["id"]])
    );
...
fn t13_wrong_dependency_reference_is_rejected_without_adopting_the_suffix() {
    ...
    let mut event =
        super::issue207_acquisition::propose(&game, Value::Null)["value"]["event"].clone();
    ...
    assert_eq!(
        replay(&before)["currentStep"]["actionRef"]["actionId"],
        "learnTwin"
    );
}
#[test]
fn t13_registered_action_scope_matches_the_complete_night_contracts() {
    use crate::contracts::FirstNightActionRef;
    let mut actual: Vec<String> = crate::first_night::catalog::production_actions()
        .into_iter()
        .map(|(a, _)| match a {
            FirstNightActionRef::System { action_id } => format!(
                "system.{}",
                serde_json::to_value(action_id).unwrap().as_str().unwrap()
            ),
