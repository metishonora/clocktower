...
fn other_night_overview_keeps_juggler_in_order_before_and_after_confirmation() {
    ...
    let juggler_index = events
        .iter()
        .position(|event| event["payload"]["actionRef"] == character("juggler", "learnJuggles"))
        .unwrap();
    ...
    let night_start = events[..juggler_index]
        .iter()
        .rposition(|event| event["payload"]["input"]["kind"] == "beginNight")
        .unwrap()
        + 1;
    ...
    for count in night_start..=juggler_index + 1 {
        ...
        let positions: Vec<_> = overview
            .iter()
            .filter_map(|row| order.iter().position(|action| *action == row["actionRef"]))
            .collect();
        assert!(
            positions.windows(2).all(|pair| pair[0] <= pair[1]),
            "night order changed at prefix {count}: {positions:?}"
        );
        let juggler = overview
            .iter()
            .find(|row| row["character"] == "juggler")
            .unwrap();
        statuses.insert(juggler["status"].as_str().unwrap().to_owned());
    }
    assert_eq!(
        statuses,
        ["waiting", "current", "complete"].map(str::to_owned).into()
    );
}
...
fn cached_replay_never_accepts_modified_history_or_unvalidated_appends() {
    ...
    for append in [false, true] {
        ...
        if append {
            ...
            let event = bad["game"]["events"]
                .as_array_mut()
                .unwrap()
                .iter_mut()
                .find(|e| e["type"] == "dayConfirmed" && e["payload"]["input"]["kind"] == "vote")
                .unwrap();
...
fn scheduled_deaths_game(demon_first: bool) -> serde_json::Value {
    ...
    if demon_first {
        ...
        let ph = entries
            .iter()
            .position(|a| *a == character("pitHag", "changeCharacter"))
            .unwrap();
        let imp = entries
            .iter()
            .position(|a| *a == character("imp", "attackPlayer"))
            .unwrap();
        entries.swap(ph, imp);
    }
...
fn scheduled_deaths_wait_for_attacks_and_undo_independently_with_death_followups() {
    ...
    assert_eq!(
        deaths["payload"]["result"]["sourceEventIds"],
        json!([change["id"]])
    );
    assert_eq!(replay(&game)["nightDeaths"]["status"], "resolved");
    assert_eq!(
        replay(&game)["nightDeaths"]["pendingAttackEventIds"],
        json!([])
    );
    ...
    forged["game"]["events"]
        ...
        .unwrap()
        .last_mut()
        .unwrap()["payload"]["result"]["sourceEventIds"] = json!([attack["id"]]);
...
fn scheduled_deaths_do_not_rewrite_an_attack_before_activation() {
    ...
    assert!(
        !replay(&game)["currentStep"]["requiredInput"]["allowedPlayerIds"]
            .as_array()
            .unwrap()
            .contains(&json!("p4"))
    );
...
fn scheduled_death_order_is_required_unique_and_after_the_trigger() {
    ...
    let order = game["game"]["script"]["definition"]["otherNightOrder"]
        .as_array()
        .unwrap();
    let index = order
        .iter()
        .position(|a| a["actionId"] == "resolveNightDeaths")
        .unwrap();
    assert_eq!(order[index - 1], character("fangGu", "attackPlayer"));
    for case in ["missing", "duplicate", "before", "version"] {
        let mut bad = game.clone();
        let entries = bad["game"]["script"]["definition"]["otherNightOrder"]
            .as_array_mut()
            .unwrap();
        match case {
            "missing" => {
    ...
    step(
        &mut unused,
        character("pitHag", "changeCharacter"),
        json!({"playerIds":["p4"],"characterIds":["soldier"]}),
        None,
    );
    step(
        &mut unused,
    ...
    assert_eq!(
        replay(&unused)["currentStep"]["actionRef"],
        character("empath", "learnEvilNeighbors")
    );
}
...
fn moving_death_resolution_after_information_preserves_the_earlier_reveal() {
    ...
    let empath = order
        .iter()
        .position(|a| a["characterId"] == "empath")
        .unwrap();
    ...
    let reveal_before =
        crate::confirmed_event_reveal_json(&game.to_string(), info["id"].as_str().unwrap());
    step(
        &mut game,
        system("resolveNightDeaths"),
